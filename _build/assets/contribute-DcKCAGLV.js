import{M as i}from"./index-BosUlz4E.js";import{b as n,q as o}from"./web-uzQsy4d_.js";const d={title:"NvChad Contributing guide",desc:"Contributing to NvChad"};function r(t){const e={a:"a",h2:"h2",li:"li",p:"p",ul:"ul",...i(),...t.components};return[n(e.h2,{children:"Things to know before contributing"}),`
`,n(e.ul,{get children(){return[`
`,n(e.li,{get children(){return[`
`,n(e.p,{children:"When making a PR (pull request), please be very descriptive about what you've done."}),`
`]}}),`
`,n(e.li,{get children(){return[`
`,n(e.p,{children:"We are open to all PRs, but may decline some for many of reasons. Don't be discouraged, though! We're still open to discussion."}),`
`]}}),`
`,n(e.li,{get children(){return[`
`,n(e.p,{children:"PR's are always welcome; however, NvChad aims to be less bloated. So PR's regarding an existing plugin's enhancement and creating new features with existing plugins itself (without adding a new plugin), bug fixes, and corrections are more encouraged."}),`
`]}}),`
`,n(e.li,{get children(){return[`
`,n(e.p,{children:"NvChad won't keep adding more and more features (like adding new plugins) as requested if they feel unneeded and aren't usable by the majority. If you think the plugin you want to be added is very useful and many NvChaders would find it useful, then such PR's are welcome."}),`
`]}}),`
`]}}),`
`,n(e.h2,{children:"Format your PR with StyLua"}),`
`,n(e.ul,{get children(){return[`
`,n(e.li,{get children(){return["Check the ",n(e.a,{href:"https://github.com/NvChad/NvChad/blob/v2.0/.stylua.toml",children:"StyLua config"})]}}),`
`]}}),`
`,n(e.h2,{children:"Other ways to contribute"}),`
`,n(e.ul,{get children(){return[`
`,n(e.li,{children:"Help other nvchad users on GitHub discussions, issues, our discord/matrix server, telegram group."}),`
`,n(e.li,{children:"Star NvChad GitHub repo."}),`
`,n(e.li,{get children(){return["If you would like to appreciate siduck's work ( creator of NvChad ) via funding, then ",n(e.a,{href:"https://github.com/NvChad/NvChad#gift_heart-support",children:"check"}),"."]}}),`
`]}})]}function l(t={}){const{wrapper:e}={...i(),...t.components};return e?n(e,o(t,{get children(){return n(r,t)}})):r(t)}export{l as default,d as meta};
